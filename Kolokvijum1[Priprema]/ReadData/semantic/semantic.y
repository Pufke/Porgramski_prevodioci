...
konstante
  : literal 
	{//tip <id> i tip svih konstanti <constN> mora biti isti
		if(get_type(idindex) != get_type($1) )
			err("Tip konstante i tip promenljive nisu korespodentni!");
	}	
  | konstante _ZAREZ literal{
	if(get_type(idindex) != get_type($3) )
			err("Tip konstante i tip promenljive nisu korespodentni!");
	}
  ;
